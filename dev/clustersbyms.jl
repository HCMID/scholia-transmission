...
# ╔═╡ 53c9941c-efc8-11eb-0cd7-f3e3ea868fcd
begin
	using CSV
	using HTTP
	using DataFrames
	...
	using Plots
	using FreqTables
	md"Notebook version:  unreleased"
end

# ╔═╡ fc93b9e6-658d-4733-89aa-ccdd71a8040e
md"> # View clusters by MS document"

# ╔═╡ e8efca12-52bf-463a-9591-b6c95cc3fcd5
clustfile = "https://raw.githubusercontent.com/HCMID/scholia-transmission/main/data/clusters-8.txt"

# ╔═╡ 45ee9dc2-a027-495e-ba29-84b6bf33e179
clusterdf = CSV.File(HTTP.get(clustfile).body) |> DataFrame

# ╔═╡ d82a9b92-cb42-4a68-ad09-32afe46e6801
clusterlist = clusterdf[:,:Cluster]

# ╔═╡ 55e012f0-b63c-418b-8074-4391453a0975
texturl = "https://raw.githubusercontent.com/hmteditors/composite-summer21/main/data/topicmodelingsource.cex"

# ╔═╡ 08ebd593-f415-41d8-9edb-48514088ca97
corp = CitableCorpus.fromurl(CitableTextCorpus, texturl, "|")

# ╔═╡ a64dfaca-42ab-454e-ac1a-32a504814515
corpurns = map(cn -> cn.urn, corp.corpus)

# ╔═╡ 88f2391c-6d81-49b8-b63e-9ef16cebfa30
...
pairsdf = begin
	...
	for i in 1:length(clusterlist)
		urn = corpurns[i]
		parts = split(workcomponent(urn), ".")
		siglum = parts[2]
		push!(sigla,siglum)
		
		push!(assignments, clusterlist[i])
	end
	df = DataFrame()
...
# ╔═╡ 7d50fddf-b455-4638-b538-e889327740cb
noe3 = filter(r -> r.siglum != "e3", pairsdf)

# ╔═╡ 1e8109c4-e44b-44f5-994d-86db61f1da95
xfreqs = freqtable(noe3,:assignments, :siglum)

# ╔═╡ 8f940a23-fede-4864-a97e-e49edf08856f
prop(xfreqs)

# ╔═╡ 98a15525-c24a-419e-9d6c-f9149bd7b19a
begin
	msnames = names(xfreqs)[2]
	
end

# ╔═╡ 0801861f-3e92-480f-9a71-1f0a989fa3a0
df

# ╔═╡ 77e3d24c-5221-4db2-a594-5e6dfe848e7c
groups = groupby(df, :siglum)

# ╔═╡ 6681b4dc-2953-4608-8f1b-487efefecbb1
DataFrames.combine(groups, :assignments => length)
