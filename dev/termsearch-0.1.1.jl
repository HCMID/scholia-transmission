...
- if you want to change MSS, you must first empty the `Term` query box
- search is for complete "terms" (tokens), not substrings
"""
...
# ╔═╡ e8561ba1-f629-424a-b292-dfbc98cc0f3d
alarm(text, label) = Markdown.MD(Markdown.Admonition("warn", label, [text]))

# ╔═╡ 9d6c8ea7-0330-4895-a88d-325ce70b8b6f
alarm(md"""`formatscholion` function is retrieving the wrong scholion although 
	
	""", "Bugs")

# ╔═╡ 3d7bafa0-1819-4944-8648-fc26da7213b6
	hint(text, label) = Markdown.MD(Markdown.Admonition("note", label, [text]))
...
function formatscholion(i)
	...
	scholurn = comments[i].urn # THIS IS WRONG.  `i` is the index into `corp.docs` and that gets updated with MS selections
...
# ╔═╡ ee0ec436-57a1-4ed7-bced-b5b3d0e4f397
...
msnodes = begin
	...
	nodesforMs(commentnodes)
end

# ╔═╡ bba171b2-f0a9-4172-a7e9-5d365a1b4f22
srcdocs = begin
	
	txtforcomments(msnodes)
end

# ╔═╡ b4b990a6-163f-485f-bd25-b214f62b5ce1
srcdocs |> length
...
begin
	...
	for idx in documentindices
		...
		hilited = highlight(term, corp.documents[idx].text) # this is right.  Need filtered list.
		formatted = formatscholion(idx)	 # But this comes out wrong???
