...
# ╔═╡ 76149d2e-4777-41a0-be9a-51436e2844a1
begin
	using TSne, Statistics, MLDatasets
	using Plots
end

# ╔═╡ ea45ce2c-e94e-11eb-08d3-43f5ec9dfd3e


# ╔═╡ 0f64db6e-55e1-49b1-94c9-86145d3be67e
md"T-SNE plot"

# ╔═╡ cdd27a10-389d-4687-a40a-6189fb7c2cf7
rescale(A; dims=1) = (A .- mean(A, dims=dims)) ./ max.(std(A, dims=dims), eps())


# ╔═╡ 8c70ca1b-43b3-433c-8f8d-f8490970fce9
alldata, allabels = MNIST.traindata(Float64)

# ╔═╡ db60cd96-c528-4c97-9803-9b99b1a97c39
data = reshape(permutedims(alldata[:, :, 1:2500], (3, 1, 2)),
               2500, size(alldata, 1)*size(alldata, 2))

# ╔═╡ 3087b43c-c216-46eb-84ed-32450d9f75cb
X = rescale(data, dims=1);

# ╔═╡ 0545ca74-2bef-4600-b5ce-125adf5b695a
Y = tsne(X, 2, 50, 1000, 20.0);

# ╔═╡ 637f5341-39c3-4509-857d-c3fdbc975fc3
p = scatter(Y[:,1], Y[:,2], marker=(2,2,:auto,stroke(0)), color=Int.(allabels[1:size(Y,1)]))
...
[[TSne]]
deps = ["Distances", "LinearAlgebra", "Printf", "ProgressMeter", "Random", "Statistics", "Test"]
git-tree-sha1 = "4a1721f9c0e4dae699c3394bfc60bf88e04f39a6"
