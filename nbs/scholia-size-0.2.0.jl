...
# ╔═╡ 2511b4d8-eb45-11eb-1fe3-134074818861
begin
	using CitableCorpus
	using CitableText
	using PlutoUI
...
# ╔═╡ fa4c1d48-010d-43ff-a2a1-9c7e2634f8f6
md"""> # Compute size of scholia
>
> Compute word count for each scholion.
> Includes options to filter for certain books, and to download counts as a delimited-text file.
"""

# ╔═╡ 254779c9-50ef-47d7-bfb3-b43cd878852c
md"> Download text corpus and filter to select scholia comments"

# ╔═╡ 21ecbf83-24b2-4e62-881e-94290ada4ade
url = "https://raw.githubusercontent.com/hmteditors/composite-summer21/main/data/archive-normed.cex"

# ╔═╡ d6a3cbb0-e41c-4d7e-9cdd-a13285925c31
c = CitableCorpus.fromurl(CitableTextCorpus, url, "|")

# ╔═╡ 2dc099fa-6f02-41ef-9906-228248da7ef3
# Comment nodes from all HMT scholia
scholia = begin
	#if bkchoice == "1-24"
		filter(cn -> occursin("comment", cn.urn.urn),  c.corpus)
	#else
	#	["SUBSET"]
	#end
end

# ╔═╡ 6ad5da6d-efb6-45b5-b88c-466775d89676
# Create a list pairing URNs with counts of "words" in each scholion
sizes = begin
	sizepairs = []
	for cn in scholia
		wdcount = split(cn.text) |> length
		push!(sizepairs, string(cn.urn.urn, "|", wdcount))
	end
	sizepairs
end

# ╔═╡ 6d0814de-ca90-42ac-9526-d638a31a86da
begin
	bklist = ["1-24","8 only","8-10", "omit book 8"]
	delimited = join(sizes,"\n")
	
	md"""
- Filter by book: $(@bind bkchoice Select(bklist))	
- Download delimited file $(DownloadButton(delimited,"scholialengths.cex"))
"""	
end

# ╔═╡ 7cff9218-3a0b-4e26-a0d1-307d7394bc58
bkscholia = begin	
	if bkchoice == "1-24"
		scholia
	elseif bkchoice == "8 only"
		filter(cn -> startswith(passagecomponent(cn.urn), "8"), scholia)
	elseif bkchoice == "8-10"
		filter(cn -> startswith(passagecomponent(cn.urn), r"[89]|10"), scholia)
...
# ╔═╡ 9b66300e-9d70-4a59-af57-022bb707ac78
begin 
	msg = string("Computed lengths for **", length(bkscholia), "** scholia filtered for book selection: **", bkchoice, "**")
	Markdown.parse(msg)
end
